...
def get_station_obs_dict(station_record):
    """
    Get a dictionary of observations for a single station record.

    Dictionaries have two metadata keys (i.e., station_id_col and "geometry") and a key for each
    measured variable.
...
def process_filepaths(data_filepaths):
    """
    Process list of JSON filepaths with Netatmo API responses into a single DataFrame.
    """
    ts_df_dict = {}
